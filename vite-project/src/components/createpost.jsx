...
const tgsElement=useRef();

const handleSubmit=(event)=>{
  event.preventDefault();
  const userId=userIdElement.current.value;
  const postTitle=postTitleElement.current.value;
  const postBody=postBodyElement.value;
  const reactions=reactionsElement.current.value;
  const tags=tgsElement.current.value.split(" ");

  userIdElement.current.value="";
  postTitleElement.current.value="";
  postBodyElement.current.value="";
  reactionsElement.current.value="";
  tgsElement.current.value="";
  ...
  return(<form className="create-post"onSubmit={handleSubmit}>
      ...
      <div className="mb-3">
        ...
        <input type="text" ref={tgsElement}className="form-control" id="tags" placeholder="Please enter your tags using space" />
      </div>
